...
// Derive usage from recent box-score volume: usg ≈ (FGA + 0.44*FTA + TOV) / possessions.
// Shot volume is available and highly predictable (split-half reliability r=0.86) even
// when the season usage rate is missing or stale. This keeps the possession core ALIVE
// for thin-data players (rookies, role-changers) instead of collapsing to the stale-PPG
// rate core, and it captures role EXPANSION that a season usage stat lags — the exact
// failure that underprojected the loss-night rookies by 74–161%.
function usageFromVolume(player, env, cfg) {
  const fga = num(player.fgaRecent) ?? num(player.last5_fga) ?? num(player.fgaAvg) ?? num(player._raw?.FGA) ?? num(player.fga);
  if (fga == null || fga <= 0) return null;
  const fta = num(player.ftaRecent) ?? num(player.last5_fta) ?? num(player._raw?.FTA) ?? num(player.fta) ?? 0;
  const tov = num(player.tovRecent) ?? num(player.last5_tov) ?? num(player._raw?.TOV) ?? num(player.tov) ?? 0;
  const min = num(player.minutesLast5) ?? num(player.minutesAvg) ?? num(player.expectedMinutes);
  if (min == null || min <= 0) return null;
  const pacePerMin = (num(env?.projectedPace) ?? cfg.leagueAvgPace) / cfg.regulationMinutes;
  const possessions = min * pacePerMin;
  if (possessions <= 0) return null;
  const used = fga + 0.44 * fta + tov;
  return clamp(used / possessions, [0.05, 0.45]);
}
...
function possessionCore(player, env, cfg) {
  ...
  let usage = normalizeUsage(player.usageRate);
  const volUsage = usageFromVolume(player, env, cfg);
  // Usage is the ONLY thing that kills this core (TS has an FG% proxy). Recent shot
  // volume is available and predictable, so use it: keep the core alive when season
  // usage is missing, and blend toward recent volume (using the engine's own recent-form
  // weight) so an expanding role isn't hidden behind a stale season stat.
  const rw = num(cfg.weights?.recentFormBlend) ?? 0.35;
  if (usage == null) usage = volUsage;
  else if (volUsage != null) usage = clamp((1 - rw) * usage + rw * volUsage, [0.05, 0.45]);
