...
function gamesByTeam(schedule) {
  ...
  for (const g of schedule || []) {
    ...
    idx[g.home.abbr] = { opponent: g.away.abbr, gameId: g.eventId, date: g.date, isHome: true, spread: homeSpread, total: g.total ?? null };
    idx[g.away.abbr] = { opponent: g.home.abbr, gameId: g.eventId, date: g.date, isHome: false, spread: homeSpread == null ? null : -homeSpread, total: g.total ?? null };
  }
  return idx;
}
...
export async function analyzeSlate(io) {
  ...
  for (const pl of players) {
    ...
    m.gameId = game.gameId; m.date = date; m.gameTotal = game.total ?? null; m.spread = game.spread ?? null;
