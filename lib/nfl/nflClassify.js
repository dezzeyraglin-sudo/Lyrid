...
export function classifyProp({ comp, volume, script, line, structure = 'standard_3', extraNudges = 0, pick = 'higher' }) {
  ...
  if (volume && volume.volume_floor_score != null && volume.volume_floor_score >= VOLUME_MIN) {
    ...
    out.reasons.push(`volume-secure (floor ${Number(volume.volume_floor_score).toFixed(2)}${volume.archetype ? `, ${volume.archetype}` : ''})`);
  } else {
  ...
  if (allPass) {
    // GUARANTEED removed — it graded as the WORST tier (inverted confidence). Nothing ships as
    // GUARANTEED; the top tier is PLATINUM, and even that is provisional until it clears breakeven.
    if (pAdj >= TIER_PROVISIONAL.PLATINUM.minP && out.edge >= TIER_PROVISIONAL.PLATINUM.minEdge) out.tier_candidate = 'PLATINUM';
