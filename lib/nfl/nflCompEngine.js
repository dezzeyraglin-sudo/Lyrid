...
export function compProject({ target, pool, line, k = 60, weights = DEFAULT_WEIGHTS, bandwidth = null }) {
  // Prefer same-position comps, but FALL BACK to the family-wide pool when they're too thin.
  // Off-position targets in a family — a TE or pass-catching RB for receiving, a QB for rushing —
  // have few same-position rows in that family's pool, which used to return null ('no comp
  // projection'). The feature vector already conditions on VOLUME/role, so the kNN by feature
  // distance still finds comparable-usage games across positions; the family pool gives a valid
  // empirical distribution rather than nothing.
  const withOutcome = pool.filter(p => p.outcome != null);
  const samePos0 = withOutcome.filter(p => p.position === target.position);
  let usePool = samePos0, offPosition = false;
  if (samePos0.length < 20) {
    if (withOutcome.length < 20) {
      return { pOver: null, median: null, lineSoftness: null, n: withOutcome.length, reason: 'insufficient comp pool (<20)' };
    }
    usePool = withOutcome; offPosition = true;   // family-wide fallback (feature distance still matches by role)
  }

  // distances
  const scored = usePool.map(p => ({ d: weightedDistance(target.features, p.features, weights), y: p.outcome }));
  ...
  return {
    ...
    offPosition,
    reason: offPosition ? 'ok (family-wide comps — few same-position)' : 'ok',
  };
}
