...
// CONTEXT-CONDITIONED channels. Weights are PROVISIONAL — fit them on the projection
// audit (the environment-change slice tells us how much each term actually predicts).
// The target vector is composed at projection time as: the player's own SKILL history
// + TONIGHT'S environment (current starter's QB channel + current team profile) +
// tonight's milestone state — which is what makes both stale-low and stale-high reads
// self-correct without rewriting history.
const DEFAULT_WEIGHTS = {
  // SKILL — what the player controls (travels with him across teams/QBs)
  volume_floor: 1.0,        // the winning-style anchor
  recent_form: 0.6,         // trailing yardage z in the family
  skill_tshare: 0.7,        // trailing target share (receiving)
  skill_ays: 0.4,           // trailing air-yards share (receiving)
  skill_carry: 0.7,         // trailing carries (rushing)
  // ENVIRONMENT — team-rolling (slow-moving)
  env_proe: 0.5,            // pass-rate over expected
  env_pace: 0.4,            // plays / pace
  env_passblock: 0.4,       // protection (inverted sack rate)
  // ENVIRONMENT — QB-specific (fast-moving; swapped when the starter changes)
  env_qb_adot: 0.4,
  env_qb_deepconnect: 0.6,  // does the QB actually complete deep — the Jefferson/McCarthy axis
  env_qb_cpoe: 0.5,
