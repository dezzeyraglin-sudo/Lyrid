...
export function pressureBattle({ offense, defense, qb }) {
  ...
  // Protection: the team table has sack_pct_allowed, NOT pressure_pct_allowed — so derive a
  // pressure-allowed estimate from the sack rate (pressure runs ~2.1x sack rate) rather than
  // silently defaulting every offense to the league constant (which made BOTH teams read
  // 'protection wins'). Prefer a real pressure_pct_allowed if a future build adds one.
  const offPressAllowed = (offense.pressure_pct_allowed != null)
    ? offense.pressure_pct_allowed
    : (offense.sack_pct_allowed != null ? clamp(offense.sack_pct_allowed / LG.sack_per_pressure, 0.12, 0.55) : LG.pressure_pct);
  // z of each side vs league; positive margin = OFFENSE wins the trench battle
  const offZ = (LG.pressure_pct - offPressAllowed) / 0.06;
  ...
  if (edge === 'offense') reasons.push(`protection holds (est. ${(offPressAllowed*100).toFixed(0)}% pressure allowed) — QB should get clean looks; favors the passing game`);
