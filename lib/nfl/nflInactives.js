...
export async function fetchAvailability(date) {
  const out = { ok: false, byPlayer: {}, games: [], fetchedAt: new Date().toISOString(), error: null, sources: [] };
  ...
  try {
    ...
    } catch (_) { /* fall through to summaries */ }
    ...
    out.ok = out.sources.length > 0 || out.games.length > 0;
  } catch (e) {
