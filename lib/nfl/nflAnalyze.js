...
export function analyzeProp(ctx) {
  ...
  // ---- PASS-GAME RECALIBRATION (from mining 166 QB/WR over losses) ----
  // The loss data proved two things about pass-game props: (1) the FLOOR is too high — 56% of
  // pass-over LOSSES fell below the model's own p25 vs 3% of wins; (2) big projected edges LOSE
  // (5-35yd edges lost 62-67%), i.e. the market is sharper than the model and the model over-
  // projects. Fix both: regress the median toward the line proportional to the edge (trust the
  // market where we're proven wrong), and drop p25 so the downside is realistic. This shades
  // over-confidence down and, where the corrected median crosses below the line, the read flips to
  // UNDER on its own (surfaced by the under-candidate logic) — turning the loss data into unders.
  let passRecal = null;
  if (comp && comp.median != null && (propFamily === 'passing_yards' || propFamily === 'receiving_yards')) {
    const edge0 = comp.median - line;
    const before = comp.median;
    if (edge0 > 4) {                                            // only regress genuine overs
      const frac = propFamily === 'passing_yards' ? 0.30 : 0.35;
      comp.median = +(comp.median - edge0 * frac).toFixed(1);   // regress a third of the edge to the market
      if (comp.mean != null) comp.mean = +(comp.mean - edge0 * frac).toFixed(1);
    }
    if (comp.p25 != null) comp.p25 = Math.round(comp.p25 - (comp.median - comp.p25) * 0.30);  // fatten the too-high floor
    comp.lineSoftness = +(comp.median - line).toFixed(1);
    if (comp.p25 != null && comp.p75 != null && comp.p75 > comp.p25) {
      const sd = (comp.p75 - comp.p25) / 1.349, z = (line - comp.median) / (sd || 1);
      comp.pOver = +(1 / (1 + Math.exp(1.702 * z))).toFixed(4);
    }
    if (before !== comp.median) passRecal = { from: before, to: comp.median };
  }
  ...
  if (passRecal) { comp.passRecal = passRecal; result.passRecal = passRecal;
    (result.narrative.drivers ||= []).unshift({ f: `pass-game recalibration: proj ${passRecal.from}\u2192${passRecal.to} (market sharper than model on pass yards)`, dir: '-', w: 0.12 }); }
