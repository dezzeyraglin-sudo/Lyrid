...
export function analyzeProp(ctx) {
  ...
  let cappedWhy = null;
  if (comp && comp.pOver != null && (propFamily === 'rushing_yards' || propFamily === 'rush_rec_yards') && volume && volume.archetype === 'committee') {
    let pen = propFamily === 'rush_rec_yards' ? 0.05 : 0.07;           // rush+rec has a mild receiving cushion
    const cm = volume.detail && volume.detail.carryMean;
    if (cm != null && cm < 13) pen += 0.03;                            // thin carry volume caps the floor further
    comp.pOver = +Math.max(0.05, comp.pOver - pen).toFixed(4);
    cappedWhy = 'capped opportunity — committee floor' + (cm != null ? ` (${cm.toFixed(0)} car/g)` : '') + `, over confidence \u2212${(pen*100).toFixed(0)}pts`;
  }
