...
// Count of PrizePicks demon/goblin ALT lines dropped this run (standard-only rule).
let _altDropped = 0;
export function getAltLinesDropped() { return _altDropped; }
export function clearAltLinesDropped() { _altDropped = 0; }
...
export function parsePrizePicks(json) {
  ...
  // Keep ONE row per (player, prop family): the FULL-GAME standard line. PrizePicks
  // returns multiple projections per player for the same stat — full game plus
  // segment lines (1H/1Q, e.g. a 57.5 full-game rush also listed at 20.5 for the half).
  // Full-game yardage is always >= any sub-period, so we dedup by keeping the MAX line
  // per (player, family). That's robust even when the segment label is opaque.
  const byKey = {};
  for (const d of json.data) {
    const a = d.attributes || {};
    const family = normFamily(a.stat_type);
    if (!family) continue; // yardage only
    // STANDARD LINES ONLY — drop demon (juiced) / goblin (discounted) alt lines.
    const oddsType = String(a.odds_type || 'standard').toLowerCase();
    if (oddsType !== 'standard') { _altDropped++; continue; }
    // Drop PROMOTIONAL lines (flash sale / free square / discounted) — giveaway numbers
    // the engine would read as a fake GUARANTEED edge (a 229.5 pass line shown at 0.5).
    if (a.is_promo === true || a.flash_sale_line_score != null || Number(a.discount_percentage) > 0 || a.discount_name) { _altDropped++; continue; }
    // Drop non-full-game SEGMENT lines (1H/2H/quarters) — the engine models full games only.
    const label = String(a.stat_display_name || a.stat_type || '');
    if (/(^|[^a-z])(1h|2h|1q|2q|3q|4q|h1|h2|q1|q2|q3|q4)([^a-z]|$)|first\s*half|second\s*half|1st\s*half|2nd\s*half|first\s*quarter|1st\s*quarter/i.test(label)) { _altDropped++; continue; }
    const line = Number(a.line_score);
    if (!Number.isFinite(line) || line <= 0) continue;
    const playerId = d.relationships?.new_player?.data?.id || d.relationships?.player?.data?.id;
    const pname = players[playerId] || a.name || 'unknown';
    // Two-player COMBO props ("Amon-Ra St. Brown + Chris Olave") are not single-player
    // lines — the engine can't project a two-name string, so it wasted work as 'baseline
    // pending'. Drop them here (also catches PP combos flagged via a '+' stat/name).
    if (/\s\+\s/.test(pname) || a.is_combo === true || /combo/i.test(String(a.odds_type||''))) { _altDropped++; continue; }
    const row = {
      app: 'prizepicks',
      player_name: pname,
      ...
      raw_start: a.start_time || null,
      raw_stat: a.stat_type,
    };
    const key = (playerId || row.player_name) + '|' + family;
    // keep the FULL-GAME line: the largest for this player+family (segments are smaller)
    if (!byKey[key] || line > byKey[key].line) {
      if (byKey[key]) _altDropped++;   // the one we're replacing was a shorter segment
      byKey[key] = row;
    } else {
      _altDropped++;                   // this row is a shorter segment; skip it
    }
  }
