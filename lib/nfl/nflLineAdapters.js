...
export function parsePrizePicks(json) {
  ...
  for (const d of json.data) {
    ...
    // STANDARD LINES ONLY — drop PrizePicks demon (juiced) / goblin (discounted)
    // alt lines so only the true line is ever analyzed (same standing rule as the MLB board).
    const oddsType = String(a.odds_type || 'standard').toLowerCase();
    if (oddsType !== 'standard') { _altDropped++; continue; }
    // Drop PROMOTIONAL lines (flash sale / free square / discounted). PrizePicks runs
    // promos that swap the real line for a giveaway number (a 229.5 pass line shown at 0.5),
    // which the engine reads as a fake GUARANTEED edge. Only true markets get analyzed.
    ...
    out.push({
      ...
      odds_type: oddsType,               // always 'standard' now; kept for downstream/diagnostics
