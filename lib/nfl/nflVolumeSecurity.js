...
export function volumeSecurity({ games, propFamily }) {
  ...
  if (propFamily === 'passing_yards') {
    ...
    // QB volume floor: high attempts + low variance. 30+ att with low CV = secure.
    const level = Math.min(1, attMean / 38);            // 38 att ~ ceiling
    const stability = Math.max(0, 1 - attCv * 2.2);     // CV 0.20 -> ~0.56
    out.volume_floor_score = +(0.55 * level + 0.45 * stability).toFixed(3);
    out.archetype = attMean >= 33 ? 'high_volume_passer' : (attMean >= 27 ? 'mid_volume_passer' : 'low_volume_passer');
