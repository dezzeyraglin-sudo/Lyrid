...
export function buildIndex(rows, opts = {}) {
  // players[id] = { name, byKey: { 'ALL'|surface : acc }, latestRank, recent: [] }
  const players = new Map();
  const cohortAcc = new Map(); // `${bucket}|${surface}` and `${bucket}|ALL`

  // chronological so per-player recent[] is time-ordered (form + fatigue need recency)
  ...
  for (const r of sorted) {
    ...
    if (r.hasStats && r.svGms) {
      P.recent.push({ date: r.date, surface: surf, minutes: r.minutes || 0,
        acePg: r.ace / r.svGms, servePct: r.svpt ? (r.firstWon + r.secondWon) / r.svpt : null });
      if (P.recent.length > 20) P.recent.shift(); // keep last 20
    }
