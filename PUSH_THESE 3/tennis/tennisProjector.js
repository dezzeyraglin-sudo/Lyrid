...
function applyContext(prof, ctx = {}) {
  ...
  // CALIBRATION FIX: index serve-points-won rates are inflated ~3-5 pts because Sackmann data
  // includes matches vs weaker fields (Challengers/quals), and holdProb is a steep curve so a 3-pt
  // spw error becomes a ~9-pt hold error → model over-projects holds → too many long sets/tiebreaks
  // → total games over-projected (verified: Wong/Muller 6-2 6-2 = 16 games, model said 25). Regress
  // spw toward the tour mean (0.635) by 35% to de-bias. This pulls Muller's 0.70→0.66 (hold 0.90→0.85),
  // matching reality. Return rates stay as-is (they're not the inflated side).
  const TOUR_SPW_MEAN = 0.635, SPW_REGRESS = 0.40;
  const debiasedSpw = prof.servePtsWonPct - (prof.servePtsWonPct - TOUR_SPW_MEAN) * SPW_REGRESS;
  const servePtsWonPct = clamp(debiasedSpw
    + clamp(ctx.formServe || 0, -w.formServeCap, w.formServeCap)
    - fatigue * w.fatigueServe, 0.5, 0.82);
